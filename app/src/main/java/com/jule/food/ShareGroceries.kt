        ...
        Column (modifier = Modifier.padding(horizontal = 10.dp), verticalArrangement = Arrangement.spacedBy(20.dp)){
            ...
            Column {
                Row(
                    ...
                    Text(
                        stringResource(R.string.category_or_categories),
                ...
                Spacer(Modifier.height(5.dp))
                Text("Total selected entries: ${selectedCategoriesIds.sumOf { id -> groceryCategories.firstOrNull { it.id == id }?.items?.count() ?: 0 }}")
            }
            Column() {
                Text(
            ...
            Row(
                ...
                Button(
                    onClick = {
                        onShare(selectedCategoriesIds, selectedShareOption)
                    },
                    enabled = selectedCategoriesIds.isNotEmpty()
                ) {
...
fun GroceryShareSheetPreview() {
    ...
    FoodTheme {
        ShareGroceriesSheet(onDismissRequest = {}, groceryCategories = viewModel.groceryItemCategories, currentCategoryId = pizza, onShare = {_, _, -> })
    }
}
