    ...
    DefaultDialog(
        ...
        LazyColumn(verticalArrangement = Arrangement.spacedBy(10.dp)) {
            items(lists) { list ->
                ...
                val color =
                    if (selected) MaterialTheme.colorScheme.tertiary else MaterialTheme.colorScheme.surfaceColorAtElevation(
                        4.dp
                    )
                Surface(
                    onClick = { if (!selected) onSelectListId(list.id) },
                    enabled = true,
                    color = color,
