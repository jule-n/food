        ...
        Row {
            Column(modifier = Modifier.weight(1f)) {
                BasicTextFieldWithBox(
                    state = nameState,
                    placeholder = { Text(
                        text = "${if (isAddSheet) stringResource(R.string.new_tag) else stringResource(
                            R.string.tag_name)}...",
...
//                modifier = Modifier.width(maxWidth)
    ...
    if (showSelectIconDialog) {
        DefaultDialog(
            ...
            onDismissRequest = { showSelectIconDialog = false }) {
            FlowRow {
    ...
    if (showDeleteConfirmDialog) {
        DefaultDialog(
            ...
            Text(
                stringResource(R.string.are_you_sure_you_want_to_delete_this_item, nameState.text),
