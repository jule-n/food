...
fun SpecificRecipeGroceries(
    recipe: Recipe,
    addToGroceries: (List<GroceryItem>, GroceryListAddingOption, categoryId: UUID) -> Unit,
    ...
    SpecificRecipeSection(
        ...
        actionButtons = {
            IconButtonWithTooltip(
                onClick = { onOpenGroceryScreen() },
                tooltipText = stringResource(R.string.edit_groceries)
            ) {
                Icon(
                    painterResource(R.drawable.edit),
                    contentDescription = stringResource(R.string.edit_groceries)
                )
            }
            FilledExpressiveIconButtonWithTooltip(
                onClick = { showGroceryAddDialog = true },
                shapes = IconButtonDefaults.shapes(),
                enabled = recipe.groceries.isNotEmpty(),
                tooltipText = stringResource(R.string.add_groceries_to_cart)
            ) {
                Icon(
                    painterResource(R.drawable.grocery),
                    contentDescription = stringResource(R.string.add_groceries_to_cart)
                )
            }
        }
    ) {
    ...
    if (showGroceryAddDialog) {
        AddGroceriesFromRecipeDialog(
            ...
            onConfirm = { groceries, addOption, categoryId ->
                if (categoryId != null) {
                    addToGroceries(groceries, addOption, categoryId)
...
fun SpecificRecipeGroceriesPreview() {
    val groceries = remember { mutableStateListOf(
        GroceryItem("Mehl", "500g"),
        GroceryItem("Spaghetti", "")
    ) }
    val recipe = Recipe(
        name = "Recipe",
...
//    recipeViewModel.addRecipe(recipe)
    FoodTheme {
        SpecificRecipeGroceries(recipe = recipe, addToGroceries = { _, _, _ ->}, onOpenGroceryScreen = {}, groceryCategories = listOf(
            GroceryItemCategory("Default"),
            GroceryItemCategory("Vegan")
        ))
    }
