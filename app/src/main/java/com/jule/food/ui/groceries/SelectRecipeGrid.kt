...
//    val textColor = if (!isInactive) MaterialTheme.colorScheme.onBackground else MaterialTheme.colorScheme.onBackground.copy(alpha = 0.3f)
    val color = if (selected == null && !disabled) MaterialTheme.colorScheme.surfaceColorAtElevation(4.dp) else
        if (selected == null && disabled) MaterialTheme.colorScheme.primaryContainer else
